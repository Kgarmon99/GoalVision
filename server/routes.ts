...
import { 
  ...
  insertOodaOpportunitySchema,
  insertOodaDailyMoveSchema,
  insertOodaStreakSchema,
  goals,
  metrics,
  goalStatus,
  users,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // OODA Revenue Engine Routes
  
  // Get all OODA opportunities
  app.get("/api/ooda/opportunities", async (req, res) => {
    try {
      const cacheKey = "ooda:opportunities:all";
      ...
      const opportunities = await storage.getAllOodaOpportunities();
      serverCache.set(cacheKey, opportunities, CACHE_TTL.DEFAULT);
      
      res.set('X-Cache', 'MISS');
      res.json(opportunities);
    } catch (error) {
      res.status(500).json({ message: "Error fetching OODA opportunities" });
    }
  });
  
  // Get top OODA opportunities
  app.get("/api/ooda/opportunities/top/:limit", async (req, res) => {
    try {
      ...
      const opportunities = await storage.getTopOodaOpportunities(limit);
      serverCache.set(cacheKey, opportunities, CACHE_TTL.DEFAULT);
  ...
  // Create OODA opportunity
  app.post("/api/ooda/opportunities", async (req, res) => {
    try {
      const opportunityData = insertOodaOpportunitySchema.parse(req.body);
      const opportunity = await storage.createOodaOpportunity(opportunityData);
      
      serverCache.invalidateByPrefix("ooda:opportunities:");
      ...
      res.status(500).json({ message: "Error creating OODA opportunity" });
    }
  });
  
  // Update OODA opportunity
  app.patch("/api/ooda/opportunities/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const opportunityData = insertOodaOpportunitySchema.partial().parse(req.body);
      const updatedOpportunity = await storage.updateOodaOpportunity(id, opportunityData);
      
      if (!updatedOpportunity) {
        return res.status(404).json({ message: "OODA opportunity not found" });
      }
      ...
      res.status(500).json({ message: "Error updating OODA opportunity" });
    }
  });
  
  // Get today's daily move
  app.get("/api/ooda/daily-move/today", async (req, res) => {
    try {
      ...
      const todayMove = await storage.getTodaysDailyMove();
  ...
  // Get all daily moves
  app.get("/api/ooda/daily-moves", async (req, res) => {
    try {
      const moves = await storage.getAllOodaDailyMoves();
      res.json(moves);
    } catch (error) {
      res.status(500).json({ message: "Error fetching daily moves" });
    }
  });
  
  // Create daily move
  app.post("/api/ooda/daily-moves", async (req, res) => {
    try {
      const moveData = insertOodaDailyMoveSchema.parse(req.body);
      const move = await storage.createOodaDailyMove(moveData);
      
      serverCache.invalidateByPrefix("ooda:daily-move:");
      
      res.status(201).json(move);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid daily move data", errors: error.errors });
      }
      res.status(500).json({ message: "Error creating daily move" });
    }
  });
  
  // Update daily move
  app.patch("/api/ooda/daily-moves/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const moveData = insertOodaDailyMoveSchema.partial().parse(req.body);
      const updatedMove = await storage.updateOodaDailyMove(id, moveData);
      
      if (!updatedMove) {
        return res.status(404).json({ message: "Daily move not found" });
      }
      ...
      res.status(500).json({ message: "Error updating daily move" });
    }
  });
  ...
  app.get("/api/ooda/streak", async (req, res) => {
    try {
      ...
      const streak = await storage.getOodaStreak();
  ...
  // Update OODA streak
  app.patch("/api/ooda/streak", async (req, res) => {
    try {
      const streakData = insertOodaStreakSchema.partial().parse(req.body);
      const updatedStreak = await storage.updateOodaStreak(streakData);
      
      if (!updatedStreak) {
        return res.status(404).json({ message: "OODA streak not found" });
      }
      
      serverCache.invalidate("ooda:streak");
      ...
      res.status(500).json({ message: "Error updating OODA streak" });
    }
  });
